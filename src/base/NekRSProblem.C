...
//DR NOTE: I think that this is at least a decent start, I need to provide some kind of index to the mesh velocities though, just need to make sure that it is the correct one.
          double normal_velocity =
              nrs->usrwrk[indices.mesh_velocity_x + vol_id] * mesh->sgeo[surf_offset + NXID] +
              nrs->usrwrk[indices.mesh_velocity_y + vol_id] * mesh->sgeo[surf_offset + NYID] +
              nrs->usrwrk[indices.mesh_velocity_z + vol_id] * mesh->sgeo[surf_offset + NZID];

          integral += normal_velocity * mesh->sgeo[surf_offset + WSJID];
          //std::cout << nrs->usrwrk[indices.mesh_velocity_y + vol_id] <<std::endl;
        }
      }
    }
  }
  ...
  std::cout << "TOTAL INTEGRAL IS " << total_integral << std::endl;
    ...
    for (int j = 0; j < mesh->Nfaces; ++j)
    {
      ...
      if (std::find(boundary_id.begin(), boundary_id.end(), face_id) != boundary_id.end())
      {
        ...
        for (int v = 0; v < mesh->Nfp; ++v)
        {
          ...
          nrs->usrwrk[indices.filtered_velocity_x + vol_id] = nrs->usrwrk[indices.mesh_velocity_x + vol_id] - total_integral*mesh->sgeo[surf_offset + NXID];
          nrs->usrwrk[indices.filtered_velocity_y + vol_id] = nrs->usrwrk[indices.mesh_velocity_y + vol_id] - total_integral*mesh->sgeo[surf_offset + NYID];
          nrs->usrwrk[indices.filtered_velocity_z + vol_id] = nrs->usrwrk[indices.mesh_velocity_z + vol_id] - total_integral*mesh->sgeo[surf_offset + NZID];

          //std::cout << "Mesh Velocity is " << nrs->usrwrk[indices.mesh_velocity_y + vol_id] << std::endl;
          //std::cout << "Filtered Velocity is " << nrs->usrwrk[indices.filtered_velocity_y + vol_id] << std::endl;
        }
      }
    }
  }
